...
std::vector<std::string> StandFirmCard::getCodeLines() const {
    return {"dmg = min(dmg, 15);"};
}
...
std::vector<std::string> MultiStrikeCard::getCodeLines() const {
    return {"for (int i = 0; i < 3; i++) enemy.takeDamage(0.5 * dmg);"};
}

std::vector<std::string> ExecuteStrikeCard::getCodeLines() const {
    return {
        "enemy.takeDamage(dmg);",
        "if (enemy.hp <= 0) player.energy += 2;"
    };
}
...
std::vector<std::string> ConstConvertCard::getCodeLines() const {
    return {"// Convert function to const (reduce cost)"};
}
...
void AttackCritCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        int damage = baseDmg;
...
void AttackPierceCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        int shieldPierce = target->getShield() / 2;
        target->reduceShield(shieldPierce);
        target->takeDamage(baseDmg, DamageType::PHYSICAL);
    });
}
...
void AttackPoisonCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::PHYSICAL);
        target->addStatus("POISON", 3, 3);  // 3 层中毒
    });
}
...
void AttackBurnCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::FIRE);
        target->addStatus("BURN", 3, 5);  // 3 回合，每回合 5 点伤害
    });
}
...
void AttackFreezeCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::ICE);
        if (rand() % 100 < 20) {  // 20% 概率
            target->addStatus("FROZEN", 1, 0);  // 冻结 1 回合
        }
    });
}
...
void AttackLightningCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::LIGHTNING);
        // TODO: 连锁伤害到其他敌人
    });
}
...
void AttackShadowCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::SHADOW);
    });
}
...
void AttackHolyCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::TRUE);
        player.heal(baseDmg / 2);  // 治疗 50%
    });
}
...
void AttackWeakenCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::PHYSICAL);
        target->addStatus("WEAK", 2, 25);  // 2 回合，攻击降低 25%
    });
}
...
void AttackStunCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::PHYSICAL);
        if (rand() % 100 < 15) {
            target->addStatus("STUNNED", 1, 0);  // 15% 概率眩晕 1 回合
        }
    });
}
...
void AttackEchoCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::PHYSICAL);
        target->takeDamage(baseDmg / 2, DamageType::PHYSICAL);  // 回响伤害
    });
}
...
void AttackChargeCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        ...
        chargeStacks++;
        int damage = baseDmg + chargeStacks * 5;
        target->takeDamage(damage, DamageType::PHYSICAL);
    });
}
...
void AttackSplitCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        int splitDmg = baseDmg / 2;
        target->takeDamage(splitDmg, DamageType::PHYSICAL);
        target->takeDamage(splitDmg, DamageType::PHYSICAL);
    });
}
...
void AttackPrecisionCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::TRUE);  // 无视防御
    });
}
...
void AttackSynergyCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([this](Player& player, Enemy* target, int baseDmg) {
        int bonus = player.minions.size() * 3;
        target->takeDamage(baseDmg + bonus, DamageType::PHYSICAL);
    });
}
...
void AttackMarkCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([](Player& player, Enemy* target, int baseDmg) {
        target->takeDamage(baseDmg, DamageType::PHYSICAL);
        target->addStatus("MARK", 3, 50);  // 3 回合，受到 +50% 伤害
    });
}
...
void AttackBerserkerCard::play(Player& player, Enemy* target) {
    player.setAttackFunc([this](Player& player, Enemy* target, int baseDmg) {
        int bonus = (player.getMaxHP() - player.getHP()) / 2;
        target->takeDamage(baseDmg + bonus, DamageType::PHYSICAL);
    });
}
...
void DefendIronWallCard::play(Player& player, Enemy* target) {
    player.setTakeDamageFunc([](Player& player, int& damage, DamageType type) {
        damage = std::max(0, damage - 5);  // 减少 5 点伤害
    });
}
...
void DefendReflectCard::play(Player& player, Enemy* target) {
    player.setTakeDamageFunc([target](Player& player, int& damage, DamageType type) {
        int reflect = damage / 3;
        if (target) {
            target->takeDamage(reflect, DamageType::TRUE);
        }
    });
}
...
void DefendDistributeCard::play(Player& player, Enemy* target) {
    player.setTakeDamageFunc([this](Player& player, int& damage, DamageType type) {
        if (!player.minions.empty()) {
...
void DefendFortifyCard::play(Player& player, Enemy* target) {
    player.setTakeDamageFunc([](Player& player, int& damage, DamageType type) {
        damage = std::min(damage, 15);  // 单次伤害上限 15
    });
}
...
void SummonEnhanceCard::play(Player& player, Enemy* target) {
    player.setSummonFunc([](Player& player) {
        Minion minion(20, 8);  // 强化的仆从
        return minion;
    });
}
...
void SummonMassCard::play(Player& player, Enemy* target) {
    player.setSummonFunc([this](Player& player) {
        Minion minion(10, 5);
        // 尝试召唤多个
        if (player.minions.size() < 2) {
...
void CopyPrecisionCard::play(Player& player, Enemy* target) {
    player.setCopySummonFunc([](Player& player, const Minion& original) {
...
void CopyMultiplyCard::play(Player& player, Enemy* target) {
    player.setCopySummonFunc([this](Player& player, const Minion& original) {
        Minion copy1(original);
        if (player.minions.size() < 2) {
            Minion copy2(original.getHP() / 2, original.getAttack() / 2);
...
void CopyImproveCard::play(Player& player, Enemy* target) {
    player.setCopySummonFunc([](Player& player, const Minion& original) {
        return Minion(original.getHP() * 1.2, original.getAttack() * 1.2);
    });
}
...
void MoveExtractCard::play(Player& player, Enemy* target) {
    player.setMoveSummonFunc([](Player& player, Minion& minion) {
        player.addEnergy(minion.getAttack() / 2);
    });
}
...
void MoveRemnantsCard::play(Player& player, Enemy* target) {
    player.setMoveSummonFunc([this](Player& player, Minion& minion) {
        Minion corpse(5, 0);
        player.addMinion(corpse);
    });
}
...
void MoveResonanceCard::play(Player& player, Enemy* target) {
    player.setMoveSummonFunc([](Player& player, Minion& minion) {
        player.addShield(minion.getHP() / 2);
    });
}
...
void SacrificeExplodeCard::play(Player& player, Enemy* target) {
    player.setSacrificeFunc([target](Player& player, Minion& minion) {
        if (target) {
            target->takeDamage(minion.getHP(), DamageType::FIRE);
        }
    });
}
...
void SacrificeInheritCard::play(Player& player, Enemy* target) {
    player.setSacrificeFunc([](Player& player, Minion& minion) {
        player.addAttack(minion.getAttack());
    });
}
...
void SacrificeRebornCard::play(Player& player, Enemy* target) {
    player.setSacrificeFunc([this](Player& player, Minion& minion) {
        if (rand() % 100 < 30) {  // 30% 概率
            Minion reborn(minion.getHP() / 2, minion.getAttack());
            player.addMinion(reborn);
        }
    });
}
...
void EscapeNimbleCard::play(Player& player, Enemy* target) {
    player.setEscapeFunc([](Player& player) -> bool {
        return rand() % 100 < 70;  // 70% 成功率
    });
}
...
void EscapeRearguardCard::play(Player& player, Enemy* target) {
    player.setEscapeFunc([target](Player& player) -> bool {
        bool success = rand() % 100 < 50;
        if (success && target) {
            target->takeDamage(player.getAttack() * 2, DamageType::PHYSICAL);
        }
        return success;
    });
}
